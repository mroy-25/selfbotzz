...
if(anu.participants[0] === zynn.user.jid) return zynn.sendMessage(anu.jid, teks, MessageType.text)
	...
	if(setting.picdetec === true){
	img = `https://api-yogipw.herokuapp.com/api/welcome3?name=${encodeUrl(namaewa)}&mem=${mdata.participants.length}&msg=SELAMAT%20DATANG%20DI%20GRUP%20${encodeUrl(mdata.subject)}&picurl=${shortpc.data}&bgurl=https://cdn.discordapp.com/attachments/819995259261288475/835055559941292032/style.jpg`
	}
...
Sering nimbrung dan baca rules grup`
	...
	if(setting.picdetec === true){
	img = `https://api-yogipw.herokuapp.com/api/goodbye3?name=${encodeUrl(namaewa)}&mem=${mdata.participants.length}&msg=SELAMAT%20TINGGAL&picurl=${shortpc.data}&bgurl=https://cdn.discordapp.com/attachments/819995259261288475/835055559941292032/style.jpg`
	}
            num = anu.participants[0]
            user = '@' + num.split('@')[0]
            gcname = mdata.subject
