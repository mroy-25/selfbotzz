...
module.exports = zynz = async (zynn, anu, db) => {
    ...
    try {
        ...
        if (anu.action == 'add') {
            ...
            console.log('welcome')
            if (setting.picdetec === true) {
              img = `https://beta-restapi.herokuapp.com/api/canvas/welcome?username=${encodeUrl(namaewa)}&memcount=${mdata.participants.length}&groupname=${encodeUrl(mdata.subject)}&ppurl=${shortpc.data}&bgurl=https://wallpaperaccess.com/full/33231.jpg&apikey=noapi`
            }
...
Sering nimbrung dan baca rules grup`
            ...
            if (setting.picdetec === true) {
                img = `https://beta-restapi.herokuapp.com/api/canvas/goodbye?username=${encodeUrl(namaewa)}&memcount=${mdata.participants.length}&groupname=${encodeUrl(mdata.subject)}&ppurl=${shortpc.data}&bgurl=https://wallpaperaccess.com/full/33231.jpg&apikey=noapi`
              }
            num = anu.participants[0]
            user = '@' + num.split('@')[0]
            gcname = mdata.subject
