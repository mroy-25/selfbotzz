...
if(anu.participants[0] === zynn.user.jid) return zynn.sendMessage(anu.jid, teks, MessageType.text)
    ...
    if(setting.picdetec === true){
    img = `http://hadi-api.herokuapp.com/api/card/welcome?nama=${encodeUrl(namaewa)}&descriminator=${asi}&memcount=${mdata.participants.length}&gcname=${encodeUrl(mdata.subject)}&pp=${shortpc.data}&bg=https://a.uguu.se/EJPmVdAC.jpg`
    }
...
Sering nimbrung dan baca rules grup`
               if(setting.picdetec === true){
        buff = await wa.getBuffer(img)
        return zynn.sendMessage(mdata.id, buff, MessageType.image, {quoted: finvite, caption: teks, contextInfo: {mentionedJid: [num]}})
                }
                else{
    ...
    if(setting.picdetec === true){
    img = `http://hadi-api.herokuapp.com/api/card/goodbye?nama=${encodeUrl(namaewa)}&descriminator=${asi}&memcount=${mdata.participants.length}&gcname=${encodeUrl(mdata.subject)}&pp=${shortpc.data}&bg=https://a.uguu.se/EJPmVdAC.jpg`
    }
            num = anu.participants[0]
            user = '@' + num.split('@')[0]
            gcname = mdata.subject
