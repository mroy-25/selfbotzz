...
exports.help = async(db, totalchat, ispublic, timee, date, dateIslamic, hit, ucselamat, runtime, run, prefix, wa_version, mcc, mnc, os_version, device_manufacturer, device_model, process, baterai, ttag, isprefix, shep) => { 
return new Promise(async(resolve, reject) => {
batre = baterai.baterai
isday = baterai.cas == true ? "Charging ⚡" : "Not Charged"
ispowersave = baterai.powersave == true ? "Aktif" : "Nonaktif"
const shape = '*›*'
const error = await db.showdata('error')
terror = '*F I T U R  E R R O R*\n\n'
for(let i of error){
  terror += `${shape} ${prefix}${i.cmd}\n`
}
let i = []
let giid = []
...
› Runtime : ${runtime(run)}
› Total Error : ${error.length} Fitur
...
${shape} ${prefix}tiktokstalk <username>
