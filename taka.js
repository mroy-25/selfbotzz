		...
		if (!tod.key.fromMe && !m.isBaileys) {
			...
			try {
				if (efk[0].user === sender) {
					afktime = efk[0].afktime
					afkreason = efk[0].reason
					cekafk = ms(Date.now() - afktime)
					enafk = "Anda telah kembali dari mode afk.\n"
					if (!afkreason == '') {
						enafk += `Setelah ${afkreason}\n`
					}
					enafk += `Selama _${cekafk.hours} Jam  ${cekafk.minutes} Menit  ${cekafk.seconds} Detik_`
					wa.Mentions(from, enafk, parseMention(enafk), tod)
					db.delete('afk', {
						user: sender
					})
				}
...
*${shp} Versi HP* : ${device_model}`
			...
			case 'menu':
				reg = await db.showdata('user')
				prefi = await db.showdata('prefix', {
					id: from
				})
				ispublic = publicc === true ? 'Public' : 'Self'
				var shep = shp
...
${res.desc}` : '\n*Tidak ada Deskripsi*'}
`
				await zynn.sendMessage(from, caption, text, {
					quoted: tod,
					contextInfo: {
			...
			case 'infogroup':
				tag = []
				try {
					deta = await db.showdata('antidelete', {
						id: from
					})
					if (deta[0].id === from) {
						var andel = '✅'
				...
				try {
					deta = await db.showdata('welcome', {
						id: from
					})
					if (deta[0].id === from) {
						var welc = '✅'
				...
				try {
					deta = await db.showdata('detector', {
						id: from
					})
					if (deta[0].id === from) {
						var detc = '✅'
				...
				try {
					deta = await db.showdata('antiviewonce', {
						id: from
					})
					if (deta[0].id === from) {
						var anvo = '✅'
...
› Left : ${lep}`
				try {
					...
					return zynn.sendMessage(from, fakeimage, MessageType.image, {
						quoted: tod,
						caption: teks,
						contextInfo: {
							mentionedJid: tag
			...
			case 'owner':
				...
				zynn.sendMessage(from, 'Ini Nomor Owner Saya ><', text, {
					quoted: hehex
				})
				break
			case 'caripesan':
				...
				try {
					...
					for (let i = 0; i < el.length; i++) {
						await zynn.sendMessage(from, 'Nih pesannya', text, {
							quoted: el[i]
						})
					}
				} catch (e) {
			...
			case 'approved3000years':
				...
				try {
					if (isMedia && !zynn.message.videoMessage || isQuotedImage) {
						...
						data = await ameapi.generate(command, {
							url: res
						})
		...
		case 'tomp3':
			if (isQuotedVideo || isMedia) {
				...
				exec(`ffmpeg -i ${media} ${ran}`, (err) => {
					...
					zynn.sendMessage(from, buffer, audio, {
						mimetype: 'audio/mp4',
						quoted: tod
					})
					fs.unlinkSync(ran)
		...
		case 'ytmp3':
			try {
				...
				var doc = async (link, nama) => {
					reply(mess.wait)
					zynn.sendFile(from, link, nama + '.mp3', nama, m, true, {
						fakeimage,
						asDocument: true
					})
				}
		...
		case 'ytmp4':
			try {
				...
				var doc = async (link, nama) => {
					reply(mess.wait)
					zynn.sendFile(from, link, nama + '.mp4', nama, m, true, {
						fakeimage,
						asDocument: true
					})
				}
		...
		case 'tr':
			...
			try {
				if (!args[1]) {
					...
					tra = await translate(tod.message.extendedTextMessage.contextInfo.quotedMessage.conversation, {
						to: args[0]
					})
					console.log(tra)
					reply(tra.text)
					...
					codelang = args[0]
					tra = await translate(body.slice(4 + codelang.length), {
						to: codelang
					})
					console.log(tra)
					reply(tra.text)
...
*Model* : ${i.phone.device_model}\n\n`
		...
		case 'setwelcome':
			...
			deta = await db.showdata('welcome_mess', {
				id: from
			})
			try {
				if (deta[0].id === from) {
		...
		case 'setleft':
			...
			deta = await db.showdata('left_mess', {
				id: from
			})
			try {
				if (deta[0].id === from) {
		...
		case 'ssweb':
			...
			reply(mess.wait)
			try {
				zynn.sendFile(from, `http://nurutomo.herokuapp.com/api/ssweb?url=${q}&full=true&type=png`, q + '.png', q, m, true, {
					asDocument: true
				})
			} catch (e) {
				reply(mess.error.api)
			...
			try {
				...
				if (!sizex == 'KB' && Number(size) >= 70) {
					...
					zynn.sendMessage(from, {
						...
						filename: `${data.data.judul}.${data.data.mime}`,
						quoted: tod
					})
				}
			} catch {
		...
		case 'snobg':
			...
			break
		case 'tagme':
			zynn.sendMessage(from, `@${sender.replace("@s.whatsapp.net","")}`, MessageType.text, {
				contextInfo: {
					mentionedJid: [sender]
				},
				quoted: tod
		...
		case 'getsticker':
			...
			try {
				data = await db.showdata('stickerdb', {
					cmd: q
				})
				for (let i of data) {
		...
		case 'd':
			try {
				...
				zynn.deleteMessage(from, {
					id: tod.message.extendedTextMessage.contextInfo.stanzaId,
					remoteJid: from,
					fromMe: true
				})
			} catch {}
...
${tu}`
		...
		case 'totag':
			if (m.quoted) {
				m.quoted.copyNForward(from, true, {
					contextInfo: {
						mentionedJid: groupMembers.map(i => i.jid)
					}
				})
			} else {
		...
		case 'ptl':
			...
			try {
				...
				teks += shp + ' Share : ' + hdata.share_count + '\n'
				wa.sendVideo(from, await wa.getBuffer(hdata.videourl), tod, teks)
			} catch {
				reply(mess.error.api)
			}
			break
